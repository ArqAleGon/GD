...
import {STATES,DEMO,metrics,matches} from './bim-state.js';
...
const filters=()=>({sector:$('sector').value,discipline:$('discipline').value,status:$('status').value,search:$('search').value.trim()});
...
function render(){
 ...
 $('rows').innerHTML=list.map(r=>`<tr class="${selected===r?'selected':''}"><td><button class="modelButton" data-id="${r.id}" aria-pressed="${selected===r}">${r.section} · ${r.code}<small>${r.file}</small></button></td><td>${r.discipline==='ARQ'?'Arquitectura':'Estructura'}</td><td>${r.version}</td><td>${percent(r.planned)}<div class="miniBar"><i style="width:${r.planned}%"></i></div></td><td>${percent(r.actual)}</td><td><span class="statusPill" style="--state:${STATES[r.status].color}">${swatch(r.status)}${STATES[r.status].label}</span></td><td>${r.start} / ${r.end}</td></tr>`).join('');
 $('rows').querySelectorAll('button').forEach(b=>b.onclick=()=>{const r=records.find(x=>x.id===Number(b.dataset.id));select(r);fit([r]);});
 $('empty').hidden=!!list.length;
 $('viewTitle').textContent=f.sector||'E15 + I16 + E16';
 $('detail').hidden=!selected;
 if(selected){const r=selected;$('detail').innerHTML=`<b>${r.section} · ${r.discipline} · ${r.code}</b><span class="statusPill" style="--state:${STATES[r.status].color}">${swatch(r.status)}${STATES[r.status].label}</span><p>Planificado ${percent(r.planned)} / ejecutado ${percent(r.actual)} · demo</p><p>${r.converted.toLocaleString('es')} elementos convertidos en este paquete.</p><p>${r.source}</p>`;}
...
for(const id of ['sector','discipline','status','cutoff'])$(id).onchange=()=>{render();fit();};
$('search').oninput=()=>{render();fit();};
$('appearance').onchange=applyMaterials;
$('reset').onclick=()=>{for(const id of ['sector','discipline','status','search'])$(id).value='';$('cutoff').value='2026-09-16';select(null);fit();};
...
async function load(){
 ...
 placement.models.forEach((definition,i)=>{const [start,end,actual]=DEMO[i];records.push({...definition,id:i,code:definition.source.split('-')[1],start,end,actual,...metrics({start,end,actual},$('cutoff').value)});});
